...
- The standard minimize button minimizes the GUI and every visible top-level window owned by the NX process; restoring the GUI restores those NX windows too. During export, the separate **Riduci NX** button minimizes NX while leaving the progress window visible and keeps NX minimized even when opening the next STEP would normally bring it back to the foreground. NX is restored once when conversion finishes; the final-summary panel therefore does not show a redundant **Riduci NX** button.
