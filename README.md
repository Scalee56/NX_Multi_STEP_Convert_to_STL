...
- **Anti-overwrite protection.** If a target STL file already exists on disk, that body is skipped (not silently overwritten) and logged explicitly with a `SKIP` prefix; skipped files are counted and reported in the final summary.
- **Cross-assembly naming collisions are disambiguated automatically.** If two *different* STEP assemblies happen to contain a component with the same name, the second one used to be silently skipped by the anti-overwrite protection (same file name, treated as "already exported"). The script now tracks which STEP first claimed each component name; a later collision from a different STEP gets its file name prefixed with that STEP's name (e.g. `AssemblyB_Screw.stl`) and a note is logged explaining the rename. Names stay simple in the common case (no collision).
- **Sub-assemblies with their own bodies are no longer skipped.** A component that has both child components *and* its own bodies (e.g. a sub-assembly with extra machining/geometry applied directly to it) is exported for its own bodies too, instead of being treated as a pure container and having that geometry silently dropped.
...
- **Incremental logging.** The log file is written line-by-line as the run progresses (not only at the end), so if NX crashes or the journal is interrupted mid-batch, the log up to that point is still readable on disk.
- **Safer resource cleanup.** STL export and part-load resources are released in `finally` blocks, so a failure exporting one body doesn't leave NX resources open for the rest of the batch.
...
- The component index (`component_index.txt`) is append-only and never deduplicated: each line represents one occurrence of a component in an assembly, so repeated components keep their correct quantity across runs. It's also used to detect cross-assembly naming collisions across separate runs, not just within a single batch.
