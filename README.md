# Batch Convert STEP to STL/3MF — NX Open Journal

C# journal for NX Open that automatically converts every STEP file (`.stp` / `.step`) in an input folder into STL or 3MF mesh files, with a pre-flight conflict scan, full assembly traversal, per-occurrence export of repeated components, and separate handling of open (non-solid) bodies. Configuration, conflict resolution and the final summary are shown in **real windows**, launched as a separate `powershell.exe` process so they aren't affected by the `System.Drawing`/`System.Windows.Forms` compatibility issue that breaks an in-process `Form` on some NX installations (see "Notes").

## What it does

1. Playing the journal (**Tools > Journal > Play...**) opens a configuration window immediately, before any NX part is touched. Choose **STL** or **3MF**, confirm the pre-filled input/output folders, and optionally open "Mostra opzioni avanzate" to change tolerances or open-mesh behaviour for this run. This window (and every other one below) is shown by a `powershell.exe` process the journal launches and waits for — see "Notes" for why. On environments where PowerShell isn't usable, the journal falls back to native dialogs and asks for the output format before the folders.
...
5. When it's done (or after you cancel it — see below), a final summary window appears with how many STEP files succeeded/failed and how many mesh files were written, plus a button to open the output folder directly and a "Chiudi" button.
...
- **STL** is the compatibility default and uses NXOpen `STLCreator`.
- **3MF** is selected before the pre-flight scan and uses the native `Creator3MF` API recorded from **NX Designcenter 2512** (`DexManager.CreateCreator3mf`, `SelectBody`, `Commit`). Older NX releases that do not expose this API are not supported by this source version; use STL or adapt the exporter with a journal recorded on that release.

- **One mesh file per body.** Solid bodies are never merged: each becomes its own `.stl` or `.3mf` file (`<name>.<format>` if there's only one solid body from that source, `<name>_corpoNN.<format>` if there are several).
...
- **Repeated components are exported once per occurrence.** If the same component appears multiple times in an assembly (e.g. 4 identical screws), it's exported 4 times — once per occurrence — with an `_occNN` suffix (`Vite_occ01.3mf`, `Vite_occ02.3mf`, ...). A component that appears only once keeps a plain name, with no suffix.
- **Cross-assembly naming collisions are disambiguated automatically.** If two *different* STEP assemblies happen to contain a component with the same name, the script tracks which STEP first claimed that name; a later collision from a different STEP gets its file name prefixed with that STEP's name (e.g. `AssemblyB_Screw.3mf`) and a note is logged explaining the rename. Names stay simple in the common case (no collision).
...
- Regardless of the chosen mode, a collision **between two outputs produced by the same run** (two different sources that would land on the same file path) is always caught and skipped with a warning — the script never silently overwrites a file it just wrote in this same run. Existing files are overwritten only after the explicit **Sovrascrivi** choice; starting automatically after a clean scan does not grant blanket overwrite permission.
- Input files must have unique base names: keeping both `Part.stp` and `Part.step` in the same input folder is rejected before conversion because both would otherwise target the same mesh names.
...
| `configuredOutputFolder` | `C:\Users\AndreaScalenghe\Desktop\STL_Convert` | Default destination folder for mesh files, logs and the component index — can be changed per run in the folder dialog |
| `chordalTol` | `0.0025` | Mesh chordal tolerance, when supported by the selected exporter |
| `angularTol` | `5.0` | Mesh angular tolerance, when supported by the selected exporter |
| `exportNotClosedMeshes` | `true` | If `true`, open/non-solid bodies are exported to a dedicated subfolder; if `false`, they're ignored |
| `generateComponentReport` | `false` | If enabled, writes `report_componenti.xlsx` with previews anchored inside cells, separate `Bounding X (mm)`, `Bounding Y (mm)`, `Bounding Z (mm)` and `Volume (mm³)` columns, plus a final sum cell; a CSV is retained as fallback |
...
5. Let it run until you see the final summary window, with per-STEP-file and per-component totals plus grand totals for solid bodies, open bodies, and mesh files written (or, in fallback mode, the same summary printed to the Listing Window/log instead). Do not close NX while it's running.
6. When it's done, check the output folder (the original one, or the new timestamped one if you chose "Copia in nuova cartella"):
   - The converted `.stl` or `.3mf` files, flat or grouped into per-source subfolders depending on how many files each source produced (see "Output format and layout" above).
   - `000_Not_Closed_Mesh\` folders (top-level and/or nested inside grouped subfolders): mesh files for open/non-solid bodies, if any were found and `exportNotClosedMeshes` is `true`.
   - `log_conversione.txt`: compact final report shown by the GUI, with totals and one result block per STEP.
   - `log_conversione_dettaglio.txt`: verbose technical trace captured incrementally during the run, retained for diagnostics.
   ...
   - `component_index.txt`: persistent map of STEP file → component occurrences, used on later runs optionally to reopen known assemblies' `.prt` files directly instead of the STEP, and to detect cross-assembly naming collisions. Each successfully traversed STEP replaces its previous snapshot, so repeated batch runs do not multiply occurrence counts.
   - `report_componenti.xlsx`: optional formatted Excel workbook generated when **Genera report Excel con immagini** is enabled. It contains one row per unique component, a preview centered and anchored inside its cell, quantity, separate bounding-box columns with units in their titles, individual `Volume (mm³)` and one final cell summing all volumes (including occurrence quantities). Inch parts are normalized to millimetres/mm³. Microsoft Excel desktop is required; `report_componenti.csv` and the `report_images` folder remain available as fallback data.
...
- Grand totals: total solid bodies exported, total open bodies exported, total mesh files written across the whole run.
- The number of mesh files skipped because they already existed and the chosen mode didn't allow overwriting them (if any).
...
- `log_conversione.txt` is rewritten at completion as a compact report with totals, input/output paths, mode and one readable result block per STEP. The verbose incremental trace is preserved separately as `log_conversione_dettaglio.txt` for diagnostics.
...
- The standard minimize button now minimizes the PowerShell GUI and activates the same background mode as **Nascondi NX**: NX, **Information**, and any later NX popup are moved off-screen and hidden with `SW_HIDE`. Restoring the GUI disables that temporary background mode and restores all NX windows. During export, the dedicated **Nascondi NX** button keeps the GUI visible while hiding NX. Both paths use Win32 foreground/show/dialog hooks rather than timer polling and are cleaned up on completion, cancellation, GUI closure, or final exit.
...
- If the progress GUI was minimized, completion automatically restores, centers and activates it so the final result is immediately visible instead of remaining hidden in the taskbar.
...
- Mesh output and GUI state files are first written to temporary files in their destination directory and then moved into place, so a failed export or an interrupted status update does not expose a partially written final file. The final GUI handshake also has a timeout, preventing NX from waiting indefinitely if the summary window stops responding.
...
- The component index (`component_index.txt`) stores one line per component occurrence. Repeated components therefore retain their quantity, while processing the same STEP again atomically replaces that STEP's previous snapshot instead of appending duplicates. In "Copia in nuova cartella" mode, the new folder starts with its own empty index — it does not inherit the original folder's history.
